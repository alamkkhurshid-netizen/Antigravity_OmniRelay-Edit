...
-- 2. Drop the old function to avoid overloading ambiguity
drop function if exists public.complete_workspace_onboarding(text, text, integer, text, text, text);

-- 3. Create the updated function with p_subscription_tier
...
begin
  ...
  if p_subscription_tier not in ('standard', 'premium') then
    raise exception 'Invalid subscription tier selected.';
  end if;

  normalized_location_count := greatest(1, least(coalesce(p_location_count, 1), 5));
  
  new_organization_id := gen_random_uuid();
  insert into public.organizations (id, name, subscription_tier, extra)
  values (
    new_organization_id,
...
revoke all on function public.complete_workspace_onboarding(text, text, integer, text, text, text, text)
  from public, anon;
grant execute on function public.complete_workspace_onboarding(text, text, integer, text, text, text, text)
  to authenticated;
