...
create index if not exists app_notifications_priority_unread_idx
  on public.app_notifications(recipient_user_id,priority,escalation_level desc,created_at desc)
...
comment on function public.refresh_priority_action_alerts(uuid) is 'Deduplicates privacy-safe administrator alerts; never sends patient messages or exposes identity in notification text.';
