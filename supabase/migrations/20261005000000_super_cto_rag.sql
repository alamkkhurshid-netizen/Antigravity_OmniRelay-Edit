alter table public.rag_knowledge_items drop constraint if exists rag_knowledge_items_source_type_check;
alter table public.rag_knowledge_items add constraint rag_knowledge_items_source_type_check check (source_type in ('faq','policy','service','clinical_guidance','business_info', 'system_architecture'));

create or replace function public.match_architecture_chunks(
...
begin
  ...
  if match_count < 1 or match_count > 15
  ...
  where kc.organization_id = p_organization_id
    ...
    and item.source_type = 'system_architecture'
...
revoke execute on function public.match_architecture_chunks(public.vector, double precision, integer, uuid)
  from public, anon, authenticated;
grant execute on function public.match_architecture_chunks(public.vector, double precision, integer, uuid)
  to service_role;
