...
export async function GET(_request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const lines = [
    ...
    "",
    "DOCTORS",
    ...((data.resources ?? []).map((provider: Record<string, string>) => `${provider.name ?? "Doctor"}${provider.specialization ? ` — ${provider.specialization}` : ""}${provider.qualifications ? ` · ${provider.qualifications}` : ""}`)),
    "",
    "LOCATIONS",
    ...((data.locations ?? []).flatMap((location: Record<string, unknown>) => [`${location.name ?? "Clinic"}${location.address ? ` — ${typeof location.address === "object" && location.address !== null ? Object.values(location.address).filter(Boolean).join(", ") : location.address}` : ""}${location.phone ? ` · ${location.phone}` : ""}`, ...(location.google_maps_url ? [`Google Maps: ${location.google_maps_url}`] : [])])),
    "",
