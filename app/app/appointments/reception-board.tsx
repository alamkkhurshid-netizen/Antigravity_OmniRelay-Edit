...
export function ReceptionBoard({organizationId,appointments,resources,locations,queueEntries,nowIso}:{organizationId:string;appointments:Appointment[];resources:Option[];locations:Option[];queueEntries:QueueEntry[];nowIso:string}){
  ...
  return <section className="reception-board" id="reception-board">
    ...
    <div className="reception-filters">
      ...
      <button type="button" className="secondary-button" disabled={!selected.length||busy!==""} onClick={()=>markArrived(selected.filter(id=>rows.find(row=>row.id===id)?.status==="confirmed"))}>Mark selected arrived</button>
    </div>
    ...
    {rows.length===0?<p className="provider-note">No active appointments match this date and filter.</p>:<div className="reception-list">{rows.map(appointment=>{const entry=queueMap.get(appointment.id);return <article key={appointment.id}><label className="reception-select"><input type="checkbox" checked={selected.includes(appointment.id)} disabled={appointment.status!=="confirmed"} onChange={event=>setSelected(current=>event.target.checked?[...current,appointment.id]:current.filter(id=>id!==appointment.id))}/></label><div className="reception-patient"><b>{label(appointment.starts_at)} · {appointment.customer_name}</b><span>{resources.find(item=>item.id===appointment.resource_id)?.name??"Doctor"} · {locations.find(item=>item.id===appointment.location_id)?.name??"Chamber"}</span><small>{appointment.status.replaceAll("_"," ")}{entry?` · Token #${entry.token_number} (${entry.queue_status})`:""}{appointment.follow_up_at?` · Follow-up ${dayKey(appointment.follow_up_at)}`:""}</small></div><div className="reception-actions">{appointment.status==="confirmed"&&<button type="button" disabled={busy!==""} onClick={()=>markArrived([appointment.id])}>Arrived</button>}{["confirmed","arrived","in_consultation"].includes(appointment.status)&&!entry&&<button type="button" disabled={busy!==""} onClick={()=>assignToken(appointment)}>Assign token</button>}{entry&&<button type="button" disabled={busy!==""||entry.queue_status==="called"} onClick={()=>setServing(entry)}>{entry.queue_status==="called"?"Now serving":"Set serving"}</button>}{["confirmed","arrived","in_consultation"].includes(appointment.status)&&<><button type="button" disabled={busy!==""} onClick={()=>followUp(appointment,10)}>10D</button><button type="button" disabled={busy!==""} onClick={()=>followUp(appointment,30)}>30D</button></>}</div></article>})}</div>}
