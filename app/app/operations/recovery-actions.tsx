...
export function RecoveryActions({ incidents, recoveries }: { incidents: RecoverableIncident[]; recoveries: RecoveryAuditItem[] }) {
  ...
  return <section className="p-6 bg-white border border-slate-200 rounded-2xl shadow-sm">
    ...
    </article>)}</div> : <div className="flex flex-col items-center justify-center py-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200"><b className="text-[15px] text-slate-800">No exhausted jobs awaiting review</b><span className="text-[13px] text-slate-500 mt-1">Automatic retries have either completed or remain within their safe retry window.</span></div>}
