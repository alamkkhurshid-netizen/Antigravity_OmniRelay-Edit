  ...
  return <section className="foundation-section chamber-schedule" id="chamber-schedules">
    ...
    {!resources.length||!savedLocations.length?<p className="provider-note">Save a provider and at least one {category==="Healthcare"?"chamber":"location"} before configuring schedules.</p>:<>
      <div className="schedule-context">
        ...
        <label>{category==="Healthcare"?"Chamber":"Location"}<select value={locationId} onChange={(e)=>load(resourceId,e.target.value)}>{savedLocations.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        ...
        <label className="assignment-toggle"><input type="checkbox" checked={active} onChange={(e)=>setActive(e.target.checked)}/> Accept bookings at this {category==="Healthcare"?"chamber":"location"}</label>
      </div>
      <div className="payment-readiness" style={{marginTop: '12px', marginBottom: '12px'}}>
        <div>
          <span>GOOGLE CALENDAR</span>
          <b>{doctorCalendar ? (category==="Healthcare"?"Doctor Calendar Connected":"Provider Calendar Connected") : "Connect Personal Calendar"}</b>
          <small>{doctorCalendar ? "This provider's appointments sync directly to Google Calendar." : `Sync this specific ${category==="Healthcare"?"doctor's":"provider's"} appointments to their own Google Calendar.`}</small>
        </div>
        {!doctorCalendar ? (
      ...
      <div className="chamber-service-list"><h3>Services, pricing and payment policy</h3>{serviceRows.map((row,index)=>{const service=services[index];return <article key={row.service_id}>
        ...
        <fieldset className="payment-policy"><legend>Patient payment options</legend>
          <label><input type="checkbox" checked={row.allowed_payment_modes?.includes("pay_at_location")} onChange={()=>togglePayment(index,"pay_at_location")}/><span><b>Pay at {category==="Healthcare"?"clinic":"location"}</b><small>Collect after visit</small></span></label>
          ...
          <label className={!paymentsReady?"disabled":""}><input type="checkbox" disabled={!paymentsReady} checked={row.allowed_payment_modes?.includes("deposit_online")} onChange={()=>togglePayment(index,"deposit_online")}/><span><b>Online deposit</b><small>Balance at {category==="Healthcare"?"clinic":"location"}</small></span></label>
        </fieldset>
      ...
      <div className="session-editor"><header><div><h3>Weekly sessions</h3><p>Add morning/evening sessions separately. Overlapping work at another {category==="Healthcare"?"chamber":"location"} is rejected.</p></div><button type="button" className="secondary-button" onClick={()=>setSessions([...sessions,{resource_id:resourceId,location_id:locationId,weekday:1,start_time:"09:00",end_time:"13:00",slot_interval_minutes:15}])}>+ Add session</button></header>
        ...
        </article>)}</div>:<p className="provider-note">No {category==="Healthcare"?"chamber-specific":"location-specific"} sessions yet. Add the first session; until then the legacy provider hours remain the fallback.</p>}
      </div>
      <div className="schedule-save"><span>Changes affect new availability immediately. Existing appointments remain intact.</span><button type="button" className="primary-button" onClick={save} disabled={busy}>{busy?"Validating schedule…":`Save ${category==="Healthcare"?"chamber":"location"} schedule`}</button></div>
