...
export function WhatsAppCostCalculator({rates,variant="hero"}:{rates:Rate[];variant?:"hero"|"header"}) {
  ...
  return <><button type="button" onClick={()=>setOpen(true)} aria-label="Open WABA Calc" className={variant === "header" ? "hidden min-h-11 items-center gap-2 rounded-xl border border-slate-200/60 bg-white/60 px-4 py-2 text-sm font-bold text-slate-700 shadow-sm backdrop-blur-md transition-all hover:bg-white hover:shadow-md hover:ring-1 hover:ring-slate-200 sm:flex" : "wa-calculator-trigger"}><Calculator className={variant === "header" ? "size-4 text-emerald-600" : "size-4"}/><span>WABA Calc</span></button>
